...
export class BankAccountService {
  ...
  async createBankAccount(createBankAccountDto: CreateBankAccountDto, createdBy: string) {
  ...
  async findBankAccountById(bankAccountId: string) {
    const bankAccount = await this.bankAccountModel.findById(bankAccountId).lean().exec();
    if (!bankAccount) {
  ...
  async updateBankAccountById(bankAccountId: string, updateBankAccountDto: UpdateBankAccountDto) {
    ...
    const updatedBankAccount = await this.bankAccountModel
      .findByIdAndUpdate(bankAccountId, payload, { returnDocument: 'after', runValidators: true })
      .lean()
      .exec();
