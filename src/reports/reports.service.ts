...
export class ReportsService {
  ...
  private resolveDateRange(
    ...
    const now = new Date();
    const period =
      timePeriod ?? (fromDate || toDate ? ReportTimePeriod.CUSTOM : undefined);
  ...
  private async buildSiteWiseReport(downloadDto: DownloadReportDto) {
    ...
    return Promise.all(
      rows.map(async (row) => ({
        ...
        supervisor:
          (await this.resolveSupervisorName(
            row._id.supervisorId?.toString(),
          )) ?? '',
  ...
  async downloadReport(reportType: ReportType, downloadDto: DownloadReportDto) {
    ...
    > = {
      ...
      [ReportType.FUND_TRANSFER]: (payload) =>
        this.buildFundTransferReport(payload),
    ...
    const extension =
      downloadDto.format === ReportFormat.EXCEL ? 'xlsx' : downloadDto.format;
