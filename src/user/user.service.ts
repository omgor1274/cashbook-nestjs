...
export class UserService {
  ...
  private mergeAddressDetails(
    ...
    const normalizedCurrent = this.normalizeAddressDetails(
  ...
  private mergeEmergencyContact(
    ...
    const normalizedCurrent = this.normalizeEmergencyContact(
  ...
  private composeAddressFromDetails(
  ...
  private sanitizeUser<T extends object & { password?: unknown }>(
    user: T,
  ): Omit<T, 'password'> {
  ...
  private async generateSpecialIndex(role: UserRole): Promise<string> {
    ...
    if (!prefix) {
      throw new BadRequestException(
        'Unsupported role for special index generation',
      );
    }
  ...
  async createUser(createuserdto: CreateUserDto) {
    ...
    const specialIndex = await this.generateUniqueSpecialIndex(
      createuserdto.role,
    );
    ...
    return createdUser
      .save()
      .then((user) => this.sanitizeUser(user.toObject()));
  }
  ...
  async normalizeSpecialIndexData() {
    ...
    for (const user of usersWithNonNormalizedSpecialIndex) {
      const normalizedSpecialIndex = this.normalizeSpecialIndex(
        String(user.specialIndex),
      );
      ...
      await this.userModel
        .updateOne(
          { _id: user._id },
          { $set: { specialIndex: targetSpecialIndex } },
        )
        .exec();
    ...
    return {
      ...
      emergencyContactBackfilled:
        emergencyContactBackfilledResult.modifiedCount,
      faceRecognitionFlagBackfilled:
        faceRecognitionFlagBackfilledResult.modifiedCount,
    };
  }
  ...
  async getCurrentUserMenu(userId: string) {
    ...
    const isAdminLike = this.isAdminLikeRole(user.role);
    ...
    return {
      ...
      quickActions: {
        ...
        bankAccounts:
          isAdminLike || normalizedPermissions.bankAccountManagement,
  ...
  async updateUserByAdmin(userId: string, updateUserDto: UpdateUserDto) {
    ...
    if (updateUserDto.role !== undefined) {
      ...
      if (existingUser.role !== updateUserDto.role) {
        updatePayload.specialIndex = await this.generateUniqueSpecialIndex(
          updateUserDto.role,
        );
      }
    }
    ...
    if (updateUserDto.addressDetails !== undefined) {
      const mergedAddressDetails = this.mergeAddressDetails(
        existingUser.addressDetails,
        updateUserDto.addressDetails,
      );
      updatePayload.addressDetails = mergedAddressDetails;

      if (
        updateUserDto.pincode === undefined &&
        mergedAddressDetails.pincode !== undefined
      ) {
      ...
      if (updateUserDto.address === undefined) {
        const composedAddress =
          this.composeAddressFromDetails(mergedAddressDetails);
        if (composedAddress) {
    ...
    if (updateUserDto.pincode !== undefined) {
      ...
      const addressDetails =
        ...
        this.normalizeAddressDetails(existingUser.addressDetails);
    ...
    if (updateUserDto.faceRecognitionEnabled !== undefined) {
      updatePayload.faceRecognitionEnabled =
        updateUserDto.faceRecognitionEnabled;
    }
