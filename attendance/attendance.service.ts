...
export class AttendanceService {
  ...
  async markAttendance(markAttendanceDto: MarkAttendanceDto, markedBy: string) {
    const attendanceDate = this.normalizeStartOfDay(new Date(markAttendanceDto.attendanceDate));

    if (markAttendanceDto.entries.length === 0) {
      throw new BadRequestException('At least one attendance entry is required');
    }

    const latestEntryMap = new Map<string, (typeof markAttendanceDto.entries)[number]>();
    ...
    const workersCount = await this.userModel
      .countDocuments({ _id: { $in: workerIds.map(id => new Types.ObjectId(id)) }, role: UserRole.WORKER })
      .exec();
  ...
  async findAttendanceByDate(queryDto: QueryAttendanceDto) {
    ...
    const [items, total, presentCount, absentCount] = await Promise.all([
      ...
      this.attendanceModel.countDocuments({ ...filter, status: AttendanceStatus.PRESENT }).exec(),
      this.attendanceModel.countDocuments({ ...filter, status: AttendanceStatus.ABSENT }).exec(),
    ]);
  ...
  async deleteAttendanceById(attendanceId: string) {
    const deletedAttendance = await this.attendanceModel.findByIdAndDelete(attendanceId).lean().exec();
