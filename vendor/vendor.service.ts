...
export class VendorService {
  ...
  async updateVendorById(vendorId: string, updateVendorDto: UpdateVendorDto) {
    ...
    const updatedVendor = await this.vendorModel
      .findByIdAndUpdate(vendorId, payload, { returnDocument: 'after', runValidators: true })
      .lean()
      .exec();
  ...
  async deleteVendorById(vendorId: string) {
    const deletedVendor = await this.vendorModel.findByIdAndDelete(vendorId).lean().exec();
    if (!deletedVendor) {
      throw new NotFoundException('Vendor not found');
    }
  ...
  async getVendorBillingHistory(vendorId: string, queryDto: QueryVendorBillingHistoryDto) {
