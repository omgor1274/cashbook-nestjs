...
import { TransactionPaymentStatus, TransactionTimePeriod } from './transaction.enums';
...
export class TransactionService {
  ...
  private resolveDateRange(
    ...
    const now = new Date();
    const period = timePeriod ?? (fromDate || toDate ? TransactionTimePeriod.CUSTOM : undefined);
  ...
  private buildFilter(queryDto: QueryTransactionsDto): QueryFilter<TransactionDocument> {
    ...
    if (queryDto.location) {
      filter.location = {
        $regex: new RegExp(`^${this.escapeRegex(queryDto.location.trim())}`, 'i'),
      };
    }
    ...
    if (queryDto.search?.trim()) {
      const searchRegex = new RegExp(this.escapeRegex(queryDto.search.trim()), 'i');
      filter.$or = [
  ...
  async createTransaction(createTransactionDto: CreateTransactionDto, createdBy: string) {
    const createdTransaction = new this.transactionModel({
      ...createTransactionDto,
      paymentStatus: createTransactionDto.paymentStatus ?? TransactionPaymentStatus.PAID,
          ...
          { $match: filter },
          {
            $group: {
              ...
              internalTransferAmount: {
                $sum: {
                  $cond: [{ $eq: ['$paymentType', 'internal_transfer'] }, '$amount', 0],
                },
              },
              count: { $sum: 1 },
            },
  ...
  async updateTransactionById(transactionId: string, updateTransactionDto: UpdateTransactionDto) {
    ...
    const updatedTransaction = await this.transactionModel
      .findByIdAndUpdate(transactionId, payload, { returnDocument: 'after', runValidators: true })
      .lean()
      .exec();
  ...
  async deleteTransactionById(transactionId: string) {
    const deletedTransaction = await this.transactionModel.findByIdAndDelete(transactionId).lean().exec();
